...
This folder contains the complete backend for the GitHub Pages website. It uses Google Sheets for records, a private Google Drive folder for files deliberately uploaded through the portal, and Apps Script `MailApp` for one-time login codes. Google OAuth is not used by parents or administrators.
...
5. Open **Triggers** and delete any old `refreshMaterialLibrary` and `continueMaterialLibraryImport` triggers.
6. Select `retireLegacyDriveArchive` and click **Run** once. This removes old archive references while preserving files uploaded through Eduwave and standalone portal worksheets.
7. Select `setupEduwave` and click **Run**.
8. Approve the requested Sheets, Drive, and send-email permissions. Drive access is used only for the private resource and student-submission upload folders.
9. Select `authorizeEduwaveMail`, click **Run**, and approve the email permission. This checks quota without sending an email.

Running `setupEduwave` again is safe. It adds any missing portal columns, including assignment type and protected-submission metadata, generates a private `AUTH_SECRET` Script Property, and preserves current portal data.
...
- The operational Sheet and `Eduwave Portal Uploads` folder must remain private.

## Add and assign learning material

Use **Academy Login > Library**.

1. Open **Library** and use **Upload current material**.
2. Choose one PDF, image, Word, Excel, PowerPoint, RTF, or text file up to 8 MB.
...
Apps Script mail quotas apply to login emails. Google consumer accounts have daily recipient limits, so keep sessions active instead of requesting a code repeatedly. New uploads must be at most 8 MB; portal previews are capped at 10 MB. PDFs and images preview in the portal. Word, Excel, and PowerPoint files are delivered as authenticated downloads. Export native Google Docs or Sheets to PDF, Word, or Excel before uploading. Videos and executable/archive formats remain excluded.
