...
1. Run `setupMaterialLibraryAutomation` once after deploying this version. It upgrades the catalogue tabs, starts importing the configured master Drive folder, and ensures the hourly refresh trigger exists.
...
3. Supported files that the academy account can read inside the master folder are added automatically as unassigned material. Their Drive folder path is retained and subject/class are inferred from file and folder names when possible.
...
The background import is resumable. `Portal_DriveSync` stores pending folders and Drive continuation tokens, so a large archive continues instead of starting again. While the first backlog exists, a temporary five-minute catch-up trigger processes safe batches; it removes itself when the queue reaches zero. The permanent hourly trigger then checks for new or changed files. The portal reports supported-material counts, folders remaining, and every skipped file with its full Drive path and reason. Files shared into the master folder are accepted when the academy account can read them. Typical skipped reasons are unsupported archive/video format, Google Forms or Slides, or the 10 MB secure-delivery limit.

If the hourly trigger ever needs repair, run `installMaterialRefreshTrigger` and approve the permission prompt if Google shows one. It creates exactly one trigger for `refreshMaterialLibrary` and removes duplicates if it is run again.
