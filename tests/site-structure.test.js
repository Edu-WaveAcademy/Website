...
assert.match(script, /Upload from this device/, 'material library must offer a clear optional upload route');
assert.match(script, /Automatic material library/, 'material library must explain the persistent Drive catalogue');
assert.match(script, /Hourly refresh is active/, 'material library must show automatic refresh status');
assert.match(script, /Files Eduwave skipped/, 'material library must report unsupported files');
assert.match(script, /material-search/, 'persistent material catalogue must be searchable');
assert.match(manifest, /auth\/script\.scriptapp/, 'Apps Script manifest must allow creation of the hourly refresh trigger');
